---
permalink: /research/
title: "Research & Expertise"
author_profile: true
---
- Autonomous 3D Exploration
- Safe Motion Planning
- Multi-Agent Exploration
- Dynamic Obstacle Avoidance
- Human-aware path planning
- Predictive models for dynamic obstacles (Model-based and data-driven)
- Learning system dynamics

## Publications

Please see, [Publications](/publications/)

## PhD Coursework
- Autonomous Systems, 6hp (WASP)
- Becoming a Teacher in Higher Education, 6hp (DAL)
- Ethical, Legal and Societal Aspects of AI and Autonomous Systems, 3hp (WASP)
- The WASP Community Building Summer School 2024, 1,5hp (WASP)
- Artificial Intelligence and Machine Learning, 6hp (WASP)
- Research Education: How to Handle the New Challenges - The PhD-Student's Performance Environment, 0,5hp (LiU)
- Reinforcement Learning, 6hp (WASP)
- Deep Learning, 6hp (WASP)
- WASP Summer School on Rethinking and Rescaling LLMs 2025, 1,5hp (WASP)
- WASP Project Course, 6hp (WASP)
- Fundamentals for PhD Studies in Science and Engineering, 6hp (LiU)
- Pitch and presentation course, 3hp
- Deep Learning for Natural Language Processing, 6hp (WASP)
- AI-Assisted Research, 6hp (LiU)


## Talks, Presentations & Travel
- WARA-PS Demonstration Week. Sep, 2023.
- AI-workshop at LiU. Presented my research for brazilian deligation. Nov, 2023.
- WASP Winter Conference. Jan, 2024.
- Linköping AI Fair. Participation as exhibitor. May, 2024.
- ICRA24. Oral and Poster presentation for accpeted paper. May, 2024.
- WARA-PS Data-Collection Week. May, 2024, Yokohama, Japan.
- Presentation of Master Thesis for SAIS Master Thesis Award, SCAI. June, 2024.
- WARA-PS Demonstration Week. Sep, 2024.
- WASP Study Trip, Vancouver (ScanLine VFC (Netflix), CDM, UBC, Teledyne FLIR, EA RnD, Simon Fraser University). Sep, 2024.
- AIICS Seminar about my research (Department level). Nov, 2024.
- WASP Winter Conference. Jan, 2025.
- WARA-PS Data-Collection Week. May, 2025.
- WARA-PS Demonstration Week. Sep, 2025.
- WASP Study Trip, Los Angeles (WinstonHill, USC, JPL, (NASA), UC Irvine, Syntiant Corp, UCLA, Caltech.). Sep, 2025.
- IROS25. Oral and Poster presentation for accepted paper. Oct, 2025, Hangzhou, China.
- Presentation WASP Project Course,Human-Aware Robot Navigation for Exploration Tasks. WASP Winter Conference. Jan, 2026.
- Safe and Scalable 3D Exploration in Dynamic and Uncertain Environments. 60%-seminar. April 2026.

## Reviewer
-  Reviewer for IROS, RA-L and ARR.

## Teaching

### Courses
- TDDD92, Artificial Intelligence - Project, TA
- TDDE19 Advanced Project Course - AI and Machine Learning, TA
- TDDE13 Multi-Agent Systems, TA (Inactive)
- TDDD99 Professionalism for Engineers, TA
- AI Academy
    - Sebastian: Turtlebot4 Point Cloud and 360 camera integration. He is now working at [Dyno Robotics](https://dynorobotics.se/).
    - Axel: Prediction of pedestrian movement using Trajnet++ and Atlas benchmark. He is now working on [Andon Labs](https://andonlabs.com/).

### Master Thesis Supervision
- Sebastian Klasson, Line execution ordering of a coverage path planning algorithm for autonomous lawn mowers. 2024. Collab with [Husqvarna](https://www.husqvarna.com/se/).
- Lukas Moe, Multi-agent 3D exploration in dynamic environments, 2025. Internal.
- William Jacobsson, Collision-free Neural Path Planning for Industrial Manipulator Robots, (Co-supervisor) 2025. Collab with [Enodo Robotics](https://enodorobotics.com/).
- Mathias Ahlgren and Mårten Saltin, Continuous Learning of Language Model-Based Agents with a Focus on Research Agents, 2026. Collab with [find my factory](https://www.findmyfactory.eu/).
- Tiantian Liang, Enhancing Safety of RL based UAV Navigation under Execution Level Dynamics Uncertainty, 2026. Internal.

## Awards
- SAIS Master Thesis Award 2024 for Best Master Thesis.

## Software, Tools & Programming Languages
- ROS1
- ROS2
- Gazebo
- C++
- Python
- PyTorch
- Git
- Docker
- Linux
